...
for await (const chunk of Bun.stdin.stream()) {
    ...
    for (;;) {
        ...
        assert.equal(request.v, protocolVersion, "Parent must use protocol v4");
        ...
        saveEvidence();
        const response: Record<string, unknown> = { v: (specification.mode === "legacy-v3") ? 3 : protocolVersion,
