...
const certificateSource = String.raw`
package main
import (
 "crypto/ecdsa"; "crypto/elliptic"; "crypto/rand"; "crypto/sha256"; "crypto/x509"; "crypto/x509/pkix"
 "encoding/hex"; "encoding/json"; "encoding/pem"; "math/big"; "net"; "os"; "path/filepath"; "time"
)
type pair struct { cert, der, key []byte }
func issue(ca *x509.Certificate, caKey *ecdsa.PrivateKey, server bool) pair {
 key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader); if err != nil { panic(err) }
 ...
 if server { template.ExtKeyUsage=[]x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth}; template.IPAddresses=[]net.IP{net.ParseIP("127.0.0.1")} }
 der, err := x509.CreateCertificate(rand.Reader, template, ca, &key.PublicKey, caKey); if err != nil { panic(err) }
 encoded, err := x509.MarshalPKCS8PrivateKey(key); if err != nil { panic(err) }
...
func write(path string, value []byte) { if err:=os.WriteFile(path,value,0600); err!=nil {panic(err)} }
func main() {
 root:=os.Args[1]; if err:=os.MkdirAll(root,0700); err!=nil {panic(err)}
 caKey,err:=ecdsa.GenerateKey(elliptic.P256(),rand.Reader); if err!=nil {panic(err)}
 ...
 caDER,err:=x509.CreateCertificate(rand.Reader,ca,ca,&caKey.PublicKey,caKey); if err!=nil {panic(err)}
 server:=issue(ca,caKey,true); client:=issue(ca,caKey,false)
 write(filepath.Join(root,"ca.pem"),pem.EncodeToMemory(&pem.Block{Type:"CERTIFICATE",Bytes:caDER}))
 write(filepath.Join(root,"server.pem"),server.cert); write(filepath.Join(root,"server-key.pem"),server.key)
 write(filepath.Join(root,"client.pem"),client.cert); write(filepath.Join(root,"client-key.pem"),client.key)
...
try {
    ...
    const certificateFile = join(lab.root, "gateway-certificates.go");
    await writeFile(certificateFile, certificateSource);
    const certificateResult = JSON.parse(await run(["go", "run", certificateFile, certificates])) as { fingerprint: string };
