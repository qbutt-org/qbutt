...
function decode(packet: Buffer): { [key: string]: Value } {
...
function compact(host: string, port: number): Buffer {
...
const lab = await createLab("discovery");
...
try {
    ...
    for (let side = 0; side < 4; side++) {
        const pieces = Array.from({ length: torrent.pieceCount }, (_, index) => index).filter(index => index % 4 === side);
    ...
    const udpTrackerPort = udpTracker.address().port;
    const credentials = [0, 1].map(() => ({ username: randomBytes(8).toString("hex"), password: randomBytes(16).toString("hex") }));
    for (let side = 0; side < 2; side++) {
        proxies.push(await startProxy({ ...credentials[side]!, udp: true, targets: [
    ...
    await writeFile(configPath, JSON.stringify({ proxies: proxies.map((proxy, side) => ({
        name: `discovery-${side}`, type: "socks5", server: proxy.host, port: proxy.port, udp: true, ...credentials[side],
    })) }));
    await lab.start();
