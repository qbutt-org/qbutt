...
| **A3 — частично** | Staged analysis индексирует выбранные корни, принимает явные mappings и проверяет кандидатов в целевом v1/v2 layout; native staging fixture покрывает renamed sources и изменение содержимого при восстановленном mtime. | Переименование/перестановка и 16→64 KiB pieces проверены для v1 (`repair-index-layout-ZtykXQ`, 33 pieces, zero download); GUI mappings и отмена большого дерева проверены в 9.1. Сохранить различие v1 layout и v2 file-root proof; не обещать универсальную delta после вставки bytes. |
