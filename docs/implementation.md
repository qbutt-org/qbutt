...
Этот раздел задаёт порядок продолжения существующего goal; обязательный объём не сокращён. На момент пересмотра счётчик goal: 10 ч 25 мин 53 с. 20 сентября пользователь возобновил работу с остатком квоты 31% и разрешил продолжать до 5%: проверять остаток между этапами, сохранить запас на остановку процессов и cleanup, не использовать reset credits. Не повторять начальную разработку и доказанные проверки без изменения кода или новой причины.
...
| **C3 — частично** | Libtorrent `30fcb515...` имеет immutable UDP owners, SOCKS UDP/uTP, scoped DHT/tracker primitives; отдельные реальные socket fixtures прошли. Приложение пока отключает managed DHT/LSD/uTP/inbound/port mapping и использует первый proxy для discovery. | Подключить единые per-torrent policies к TCP/UDP/uTP, HTTP(S)/UDP trackers, DHT, DNS, webseeds и metadata discovery. Проверить generation, infohash scope, self-connection, BEP42 и tracker identity/source/port реальными запросами и packets; отдельные component tests не закрывают приложение. |
