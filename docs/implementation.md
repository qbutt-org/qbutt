...
Срез после alpha.2 и IPC v5: опубликованные результаты и точные ограничения перечислены в 9.1. Следующий локальный сетевой шаг — двухторрентный runtime-тест общего connection budget/dedup; source-аудит уже выполнен, повторять его не нужно. Следующий конкретный A2 blocker — согласованные physical mappings для in-place с selected/ignored files, incomplete suffix и раздельными save/download directories. Staging уже принимает selected subset/actual mappings, но общие AutoTMM/download-path/suffix guards пока остаются; нельзя просто убрать их без ownership/recheck контракта. Независимый WAN ingress и Koala off/on/restart остаются отдельными условиями, а не поводом объявлять полную цель выполненной. Проверенные рабочие копии с patch-equivalent изменениями удалены; ветки сохранены, остальные checkout и актуальные build/dependencies оставлены для продолжения.
...
Source-аудит libtorrent `d600f78` подтвердил сохранение общего session/torrent connection budget и dedup по исходному peer endpoint: managed dial регистрируется через штатные `insert_peer`/`set_connection`; `peer_list.cpp` не изменён относительно закреплённого upstream. Routed TCP retry повторно проверяет connection, policy/generation и лимиты, не использует `ignore_limit`. Это не runtime-доказательство budget: остаётся отдельный двухторрентный тест с повторяющимися DHT/tracker endpoints и измерением максимума активных соединений. Штатный incoming slack и ограниченные gateway handshakes не равны строгому лимиту всех ОС-сокетов.
...
| **A2 — частично** | [RepairService](../src/base/bittorrent/repairservice.cpp), [TorrentImpl](../src/base/bittorrent/torrentimpl.cpp), [SessionImpl](../src/base/bittorrent/sessionimpl.cpp): admission, drain, исключительное изменение размера через Windows handle, стандартный recheck. После read-only analysis и consent guard создаёт отсутствующие zero-length targets и каталоги; `smoke:repair` проверяет apply/recheck/download и exact sizes для v1/v2/hybrid. Исключение из `truncate_files` обосновано в ADR. | Поддержать штатные selected/incomplete/download-path mappings без текущих ограничений первого этапа; проверить передачу ownership при ошибке/отмене/перезапуске и final hashes + exact sizes после докачки на полном corpus. Не обещать rollback для in-place. |
...
| **E1–E3 — частично** | Trusted inbound libtorrent adapter и qbutt-net v4 lease подключены через [PathManager](../src/base/net/pathmanager.cpp). Whole-app TCP/uTP и входящий DHT прошли controlled IPv4/IPv6, включая original endpoint, renewal/reopen/terminal rollover. `gateway-wan-UogdS4` подтвердил remote peer-first TCP и 524 288 hash-verified bytes (9.1). | Нужны независимый third-party peer к реальному публичному endpoint за CGNAT, WAN UDP/uTP/DHT и правильные TCP/UDP announces, полная family/fault matrix. Remote gateway и peer на одном observer не доказывают внешнюю достижимость; новый independent VPN preflight пока failed. |
