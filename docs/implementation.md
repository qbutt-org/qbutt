...
| **C3 — частично** | Managed UDP/DHT, route-local bootstrap DNS и физически привязанный Native DNS реализованы; `path-dns-lyQ54q` прошёл с сохранением двух SOCKS paths и child после DNS ответа. Libtorrent валидирует BEP42 hint по endpoint/transaction/generation. Whole-app DHT + HTTP/UDP tracker union, automatic uTP→TCP retry и локальный PEX через другой path приведены в 9.1. Там же controlled IPv4/IPv6 gateway lease проверил входящий DHT и uTP с exact payload. | Нужны gateway UDP tracker и SSL-uTP, family mismatch fail-closed и оставшиеся HTTPS/webseed transitions. WAN uTP/DHT через независимый VPN ingress, public magnet widening, private scope и HTTP reconnect проверены в 9.1; они не доказывают физический обход system VPN/CGNAT. Проверить external source/port, announce и self-connection в независимой WAN-сети; controlled loopback не считать таким доказательством. |
