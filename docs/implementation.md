...
Приоритеты уточнены пользователем: сначала сетевая часть и скачивание. Repair — дополнительная задача низкого приоритета. Автообновление — в самом конце, простое скачивание с GitHub Releases по примеру `element-max`; порядок разделов и первоначальная очередность ниже не повышают приоритет этих функций.
...
Сначала T00 → T01 → T02, затем B1/B2 и сетевой D1. C2 начинается, когда B2 уже доказывает один корректный path; после него C3/C4 и E1. Автоматическая адаптация C5 появляется после измеримого и корректного ручного Mixed режима. A1/A2 остаются второстепенными; расширение repair A3/A4 не задерживает сетевые этапы. D4 выполняется последним.

Не откладывать работающий qbutt до завершения всей дорожной карты: первый самостоятельный артефакт должен доказывать один явно выбранный сетевой путь. Уже реализованный repair проверяется в его текущих границах; его расширение не является условием следующего сетевого артефакта. Каждый следующий артефакт расширяет проверенный сквозной сценарий.
