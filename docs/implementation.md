...
Срез после alpha.2 и IPC v5: опубликованные результаты и точные ограничения перечислены в 9.1. Двухторрентный runtime-тест общего connection budget, повторных explicit candidates и dedup пересекающихся DHT/tracker responses пройдены; source-аудит повторять не нужно. Остаётся incoming/reconnect budget. Следующий конкретный A2 blocker — согласованные physical mappings для in-place с selected/ignored files, incomplete suffix и раздельными save/download directories. Staging уже принимает selected subset/actual mappings, но общие AutoTMM/download-path/suffix guards пока остаются; нельзя просто убрать их без ownership/recheck контракта. Независимый WAN ingress и Koala off/on/restart остаются отдельными условиями, а не поводом объявлять полную цель выполненной. Проверенные рабочие копии с patch-equivalent изменениями удалены; ветки сохранены, остальные checkout и актуальные build/dependencies оставлены для продолжения.
