...
| **A1 — частично** | [repairanalysis.cpp](../src/base/bittorrent/repairanalysis.cpp) читает целевые v1 pieces и v2 roots/piece layers под [guard](../src/base/bittorrent/repairfileguard.cpp); worker вынесен из UI. Alpha подтверждает read-only и verified bytes для управляемой остановленной раздачи. | Pre-write Qt preview, mappings, раздельные candidate/verified/network/temp bytes, отмена и отсутствие writes до consent проверены (`repair-product-IIJhQk`). Остался выбор subset файлов в preview нового `.torrent` до consent; selected/ignored repair уже добавленной задачи не закрывает этот UI-сценарий. |
