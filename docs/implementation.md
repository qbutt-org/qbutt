...
| **A1 — реализовано, финальная композиция остаётся** | [repairanalysis.cpp](../src/base/bittorrent/repairanalysis.cpp) читает целевые v1 pieces и v2 roots/piece layers под [guard](../src/base/bittorrent/repairfileguard.cpp); worker вынесен из UI. Pre-write Qt preview, mappings, candidate/verified/network/temp bytes, отмена и отсутствие writes до consent проверены. Выбор subset до consent нового `.torrent`, передача native priorities и selected-only commit подтверждены `repair-product-HChvSc` и `qt-acceptance-5Zqo72` (9.1). | Повторить композиционный smoke на итоговом release bundle; offscreen Qt не заменяет физический desktop сценарий. |
...
| **A3 — частично** | Staged analysis индексирует выбранные корни, принимает явные mappings и проверяет кандидатов в целевом v1/v2 layout; native staging fixture покрывает renamed sources и изменение содержимого при восстановленном mtime. | Переименование/перестановка и 16→64 KiB pieces проверены для v1/v2 (`7S32Mn`/`TYhN0V`, 33/36 pieces, zero download); GUI mappings и отмена большого дерева проверены в 9.1. Осталась композиционная проверка итоговой сборки. Сохранить различие v1 layout и v2 file-root proof; не обещать универсальную delta после вставки bytes. |
...
| **C3 — частично** | Managed UDP/DHT, route-local bootstrap DNS и физически привязанный Native DNS реализованы; `path-dns-lyQ54q` прошёл с сохранением двух SOCKS paths и child после DNS ответа. Libtorrent валидирует BEP42 hint по endpoint/transaction/generation. Whole-app DHT + HTTP/UDP tracker union, automatic uTP→TCP retry и локальный PEX через другой path приведены в 9.1. Controlled gateway подтвердил IPv4 UDP tracker first contact и IPv4/IPv6 DHT/uTP; public/private HTTPS retirement сохранил TLS и exact payload. | Остались SSL-uTP и полная family mismatch/fault matrix. WAN uTP/DHT через независимый VPN ingress, public magnet widening, private scope и HTTP/HTTPS reconnect проверены в 9.1; они не доказывают физический обход system VPN/CGNAT. Проверить external source/port, announce и self-connection в независимой WAN-сети; controlled loopback не считать таким доказательством. |
| **C4 — частично** | [PathManager](../src/base/net/pathmanager.cpp) синхронно меняет Pinned/Mixed/Tunnels only и отзывает generation; child death оставляет managed blocked. `smoke:tunnels` проверяет живую задачу Pinned→Tunnels only, два пути и retry; `smoke:path-auth` проверяет отказы и terminal rollover. Private остаётся pinned; unknown ограничен pinned до получения metadata. Direct↔Tunnels TCP и Mixed→Tunnels→Pinned uTP/discovery прошли, включая закрытие Native sockets и положительный canary control (9.1). | Завершить оставшиеся webseed/family/fault combinations на итоговом bundle; сохранять private scope при отказах и смене generations. Не расширять проверенный IPv4 UDP tracker результат до всех семейств. |
| **C5/C6 — частично** | [PeerRouteSelector](../src/base/net/peerrouteselector.cpp) учитывает per-peer/path failures, demand/choke и hash-verified block credit, ограничивает exploration; двухпутевой wrong-route retry и локальный PEX B на другом path прошли. Automatic uTP→TCP retry остаётся в том же разрешённом generation. Controlled Native address retirement/reconnect и failed remote path прошли с сохранением здорового соединения (9.1). | Mixed передаёт данные через четыре реальных пути; остаются сравнительные bad-path/no-benefit/last-mile измерения и итоговые aliases/restart проверки 9.3. Controlled смена Native snapshot не заменяет физическую смену интерфейса. Reward не должен смешивать verified bytes с relay/wire или повторным recheck. |
...
| **D2/D5 — частично** | [CompletionPolicy](../src/base/bittorrent/completionpolicy.cpp) реализует правила `wanted_files_committed` с category/tag/ratio/time, preview, reason journal, idempotent claims и отдельное согласие на delete-data; `smoke:policies`/fault fixtures покрывают repair/staging/move/restart сценарии, Qt acceptance — completion stop. `remove_torrent` сохраняет payload. | Queued/nested-modal ownership и отказ native resume persistence проверены (`queued-EFIWPj`, `modal-H4Qpy6`, `staging-receipt-mappings-Tjiflz`), импорт вместе с rule engine — `profile-import-Agmms7`. Итоговый bundle требует композиционного smoke; неизменённый fault corpus не повторяется из-за tracker-only исправления. |
| **D3 — реализовано, финальная композиция остаётся** | [ProfileImport](../src/base/profileimport.cpp), [Qt dialog](../src/gui/profileimportdialog.cpp) и startup recovery копируют разрешённые настройки/metadata в отдельный профиль. Bencode/SQLite, mappings, writer exclusion, crash/backup recovery и неизменность источника проверены в 9.1. Whole-app Qt import→recheck→restart→acknowledgement→restart, включая UI-negative schema/metadata, passed (`profile-qt-OzTzia`); service/engine corpus — `profile-import-Agmms7`. | Композиционная проверка итогового bundle. Импортированные раздачи stopped/manual; source profile и глобальная сетевая политика не переносятся целиком. |
...
| **E1–E3 — частично** | Trusted inbound libtorrent adapter и qbutt-net v5 lease подключены через [PathManager](../src/base/net/pathmanager.cpp). Whole-app TCP/uTP и входящий DHT прошли controlled IPv4/IPv6, включая original endpoint, renewal/reopen/terminal rollover. Independent VPN ingress подтвердил TCP (`gateway-wan-2mGt5m`) и uTP/DHT (`gateway-wan-utp-dht-8bw1KW`), exact 524 288 bytes в каждом прогоне (9.1). Controlled HTTP/UDP lease announces и retirement прошли на финальном child (`gateway-utp-KL4S23`). | Осталась полная family/fault matrix и независимая WAN-проверка announces. Источник через отдельный VPN exit доказывает внешний вход на публичный endpoint, но не отдельный физический third-party host, CGNAT у провайдера или обход действующего system VPN. |
