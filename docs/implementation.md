...
Независимый WAN gateway preflight `gateway-wan-UBZTYY` и `gateway-wan-G1G7aX` — failed до запуска gateway: отдельный выбранный VPN adapter принял 59 bytes HTTP-запроса, но scoped observer capture не увидел SYN и HTTP listener не принял соединение. Проверены random high port 61468 и ранее успешный 40971; гипотеза только диапазона портов не подтвердилась и её временный режим удалён. Fixture теперь сначала требует доказанный внешний source IP, отличный от домашнего SSH origin и observer; SOCKS CONNECT сам по себе не считается успешным VPN ingress. Причина недоставки через выбранный adapter пока не установлена.
